...
func main() {
	if err := run(); err != nil {
		slog.Error("docsplitter service exited with error", "error", err)
		os.Exit(1)
	}
}
...
func run() error {
	...
	natsName := envOr("NATS_CLIENT_NAME", "explorer-docsplitter")
	...
	svc := docsplitter.New(logger, js, docs, blob)

	logger.Info("docsplitter service starting",
		"nats", natsURL,
		"blob_dir", blobDir,
	)
