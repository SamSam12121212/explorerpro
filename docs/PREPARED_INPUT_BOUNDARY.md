...
- 2026-04-12: Task 8 completed. Removed the leftover dead helper surface from `internal/worker/docexec.go` now that document warmup materialization lives behind prepared-input refs. Worker tests no longer recreate documenthandler-owned `<pdf>` / `<pdf_page>` markup and instead validate the worker against generic prepared-input artifacts plus blob-ref lowering. This effectively closes the Phase 5 cleanup pass while keeping worker-owned OpenAI execution and document-query session lineage intact.
