# Explorer UI styles (AgentsApp alignment)

This document defines how the Explorer frontend should **visually align** with the AgentsApp Electron app while staying **Tailwind-first** and consistent with [`frontend/TAILWIND.md`](../frontend/TAILWIND.md).

## Source of truth

| Location | Role |
| -------- | ---- |
| **AgentsApp** (`~/agentsapp`) | Reference app: Electron + React + Tailwind CSS v4 (`@import "tailwindcss"` in `src/index.css`). |
| **Explorer** (`frontend/`) | Implementation target: Vite + React 19 + Tailwind v4. |

AgentsApp global styling, scrollbars, markdown/code surfaces, and panel-specific overrides live primarily in:

- `~/agentsapp/src/index.css`

Component-level patterns use explicit hex utilities (examples: `bg-[#2a2a2a]`, `border-[#333333]`, `text-[#b2b2b2]`) alongside Tailwind layout utilities—see e.g. `src/modules/agent/components/ChatInput.tsx`, `src/components/sidebar/ChatList.tsx`.

## Reference imagery (AgentsApp repo root)

Use these as **visual references** for density, contrast, and chrome (not as runtime assets unless copied into Explorer):

| File (AgentsApp root) | Typical use |
| ----------------------- | ----------- |
...
## Design tokens (from AgentsApp `index.css`)
...
AgentsApp uses a **muted gray** for reasoning blocks (e.g. `.reasoning-markdown` at `#6d6d6d`). Reserve a named token if Explorer renders reasoning separately from main assistant text.
...
AgentsApp standardizes **thin, neutral gray** scrollbars with **no rounded thumbs** on WebKit (square corners). Patterns include:
...
AgentsApp uses ID-scoped overrides for light Tailwind grays remapped to dark:
...
AgentsApp styles `.markdown-content` for headings, lists, blockquotes, tables, inline code, and fenced blocks. When Explorer adds markdown:
...
1. **Tokens first** — Add shared colors, radii, and fonts to `@theme` in `frontend/src/styles.css`; use semantic names in JSX where it improves consistency with AgentsApp.
...
4. **Co-locate intent** — Layout and spacing stay in the component; repeated “AgentsApp card” patterns become one `@layer components` class or a tiny set of theme colors.
5. **Accessibility** — When using very dark surfaces, keep **focus-visible** styles explicit; AgentsApp removes focus rings in some sidebar areas for Electron—**web** Explorer should retain visible focus for keyboard users unless a deliberate exception is documented.
6. **Resizable panels** — Pane chrome (resize handles, borders) should use the same border token (`#333333` family) so splits match sidebar dividers in AgentsApp.
...
- [ ] Compare against `image1.png` / `image2.png` in AgentsApp root for overall contrast.
