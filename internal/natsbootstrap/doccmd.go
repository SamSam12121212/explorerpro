...
func EnsureDocCommandStream(js nats.JetStreamContext) error {
	...
	info, err := js.StreamInfo(doccmd.StreamName)
	if err == nil && info != nil {
		if _, err := js.UpdateStream(cfg); err != nil {
			return fmt.Errorf("update %s stream: %w", doccmd.StreamName, err)
		}
		return nil
	}

	if _, err := js.AddStream(cfg); err != nil {
		return fmt.Errorf("add %s stream: %w", doccmd.StreamName, err)
	}
